...
- When in doubt even slighlty, or your own knowledge may be outdated (i.e. you learned some of the concepts several years ago or recently, but used potentially outdated information) consult with documentation I downloaded from aws:
  * `aws-pdfs/` folder contains official AWS developer and user guides converted to txt format using pdftotext utility. These are the authoritative AWS documentation files you can read directly. Services covered: CloudFront, CloudWatch, API Gateway, Aurora, AWS CLI, CDK, CloudFormation, Cognito, DynamoDB, EC2, ECS, EKS, IAM, Lambda, RDS, Route 53, S3, SAM, Serverless Core, SNS, SQS, SSO, VPC
