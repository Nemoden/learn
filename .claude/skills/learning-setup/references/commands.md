...
```markdown
Show me adjacent technologies / alternatives for $ARGUMENTS project (or current project if none specified).
...
**Behavior**:
- If a category filter is given (e.g., "/adjacent ingestion" or "/adjacent storage"), show only that category.
...
- `/adjacent` — no topic-specific changes; behavior is driven by `projects/<project>/adjacent.md`
